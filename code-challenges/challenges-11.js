...
function fullName(arr) {
  const arr1 = arr.map(x => (x.firstName + " " + x.lastName))
  return arr1
  // write your code here
}
// -------------------------------------------------------------------------------------------------------
...
function gradesAvg(arr) {
  ...
  const studentsWithAvg = arr.map(student => {
    ...
    for (let i = 0; i < student.gradsList.length; i++) {
      gradesTotal += student.gradsList[i];
    }
    const avg = gradesTotal / student.gradsList.length;
    return { avg, ...student };
  });
...
function studentsResult(arr) {
  ...
  const studentsWithAvg = arr.map(student => {
    ...
    let avg
     avg = gradesTotal / student.gradsList.length;
