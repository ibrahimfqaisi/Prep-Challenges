...
const customerAndAge = (obj) => {
  ...
  for (const key in obj) {
    if (obj.hasOwnProperty.call(obj, key)) {
      let coustomerInfo = `Customer Name :${key} , Age :${obj[key]}`;
      arr.push(coustomerInfo);
    }
  }
  return arr
};
...
const getEntries = (obj) => {
  ...
  for (const key2 in obj) {
    if (obj.hasOwnProperty.call(obj, key2)) {
      let info = `${key2}: ${obj[key2]}`;
      arr2.push(info);
    }
  }
  return arr2;
...
const getInfo = (arr) => {
  ...
  for (const key of courses) {
    ...
    for (const iterator of key.Students) {
...
const getStudents = (arr) => {
  ...
  for (const iterator1 of arr) {
    for (const iterator2 of courses) {  
      for (const itertor3 of iterator2.Students)
        if (iterator1 == itertor3) {

          arr4.push({
            Student : iterator1,
          course : iterator2.course
          })
        }
