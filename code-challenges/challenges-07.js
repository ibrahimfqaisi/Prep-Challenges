...
const objLat = (obj) => {
    ...
    let firstNamee = obj.firstName.charAt(0).toUpperCase() + obj.firstName.slice(1);
    let lastNamee = obj.lastName.charAt(0).toUpperCase() + obj.lastName.slice(1);


    return (
        `my name is ${firstNamee} ${lastNamee} I am ${obj.age} YO, and I love ${obj.hobby}.`
    );
    ...
    for (let i = 0; i < arr.length; i++) {
        if (arr[i].yearsOfExperience > 1) {
            if (arr[i].firstName !== null && arr[i].lastName !== null) {
                margName = arr[i].firstName + " " + arr[i].lastName;
            } else {
                if (arr[i].firstName !== null) {
                    margName = arr[i].firstName;
                };
                if (arr[i].lastName !== null) {
                    margName = arr[i].lastName;
                };
            }


            arr2.push({ fullName: margName, tech: arr[i].tech })
...
const applicationsStatics = (arr) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        if (((arr[i].lastName !== null && arr[i].lastName !== "") || (arr[i].firstName !== null && arr[i].firstName !== "")) && arr[i].yearsOfExperience > 1) {
...
const classesAvg = (data) => {
    for (let i = 0; i < data.grades.length; i++) {
        for (let j = 0; j < data.grades[i].classes.length; j++) {
           let avrg = 0;
           let count = 0;
            for (let f = 0; f < data.grades[i].classes[j].classScores.length; f++) {
                avrg += data.grades[i].classes[j].classScores[f]
                count++
            }
            data.grades[i].classes[j].avg = Math.floor(avrg / count);
        }
